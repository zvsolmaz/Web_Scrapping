...
BASE_URL = BASE_SOURCES["yenikocaeli"]
...
def link_gecerli_mi(href: str) -> bool:
    if not href:
        return False
    if href.startswith("whatsapp:") or href.startswith("javascript:"):
        return False
    if "facebook.com" in href or "twitter.com" in href:
        return False
    if "#" in href:
        return False
    if href.startswith("http") and "yenikocaeli.com" not in href:
        return False
    if "/haber/" in href and ".html" in href:
        return True
    return False
...
def fetch_yeni_news():
    ...
    # Ana sayfa — SSL hatası olabilir
    response = safe_get(BASE_URL, session=session)
    if response is None:
        print("yenikocaeli ana sayfa hatası: bağlantı kurulamadı (SSL dahil)")
        return results
    ...
    for a_tag in soup.find_all("a", href=True):
        ...
        try:
            ...
            if time_tag:
                published_at = parse_turkish_date(time_tag.get("datetime") or time_tag.get_text())
            if not published_at:
    ...
    print(f"\nyenikocaeli toplam: {len(results)}")
    return results
